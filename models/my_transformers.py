...
class GPT2ModelForVAE(GPT2Model, ABC):
    def __init__(self, config, latent_size=64, latent_as_gpt_emb=True):
        super().__init__(config)
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            if latent_as_gpt_memory:
                past_key_values = self.linear(past_key_values)   # batch x (n_layer * hidden_size)
                
                num_att = past_key_values.shape[-1] // (self.config.hidden_size * self.config.n_layer)
                ### ori
                if num_att == 1:
                    past_split = torch.split(past_key_values.unsqueeze(1), self.config.hidden_size, dim=2) # tuple: 49  tensor: batch x 1 x hidden_size
                else:
                ### four tokens
                    past_split = past_key_values.view(past_key_values.shape[0],num_att,-1)
                    past_split = torch.split(past_split, self.config.hidden_size, dim=2)
                
                past_split = [self.h[0].attn._split_heads(past, self.h[0].attn.num_heads, self.h[0].attn.head_dim) # list: 49  tensor: batch x n_head x 1 x (hidden/n_head)
...
class GPT2ForLatentConnector(GPT2PreTrainedModel, ABC):
    def __init__(self, config, latent_size=64, latent_as_gpt_emb=True, latent_as_gpt_memory=True):
        super().__init__(config)
        self.transformer = GPT2ModelForVAE(config, latent_size=latent_size, latent_as_gpt_emb=latent_as_gpt_emb)
...
class GPT2ModelForVAENew(GPT2ModelForVAE, ABC):
    def __init__(self, config, latent_size=64):
        ...
        # self.linear = nn.Linear(self.latent_size, 4* config.hidden_size * (config.n_layer + 1) ,bias=False)
        
        #MLP2
        # self.linear = nn.Sequential(
        #         nn.Linear(self.latent_size,self.latent_size),
        #         nn.LeakyReLU(),
        #         nn.Linear( self.latent_size,  config.hidden_size * (config.n_layer + 1), bias=False),
        # )
